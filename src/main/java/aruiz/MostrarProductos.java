...
public class MostrarProductos {
    ...
    public void mostrarProductos(){
        ...
        nuevaTabla.addRule();
      // AT_Row fila1 = nuevaTabla.addRow(null,null,productos.get(0).getTipoMenu().toString().toLowerCase(),null,null);
       // fila1.setTextAlignment(TextAlignment.CENTER);
        nuevaTabla.addRule();
        AT_Row fila2 = nuevaTabla.addRow("Código", "Nombre", "Precio Base");
        fila2.setTextAlignment(TextAlignment.CENTER);
        nuevaTabla.addRule();
        for (int i = 0; i <productos.size() ; i++) {
                AT_Row fila = nuevaTabla.addRow(productos.get(i).getCode(),productos.get(i).getNombre(),productos.get(i).getPrecioBase());
